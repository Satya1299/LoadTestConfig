...
func init() {
	...
	flag.StringVar(&client_id, "clientId", "", "client_id")
	flag.StringVar(&client_secret, "clientSecret", "", "client_secret")
...
func client(configuration *Configuration, result *Result, done *sync.WaitGroup) {
	for result.Requests < configuration.requests {
		for _, tmpUrl := range configuration.urls {
			...
			randomTxnidStr := strconv.FormatInt(randomTxnid, 10)
			RandomUname := "Satya_" + randomTxnidStr
			updatedData := bytes.Replace(data, []byte("random"), []byte(RandomUname), 1)
			...
			if len(configuration.client_id) > 0 {
				req.Header.Set("client_id", configuration.client_id)
			}
			if len(configuration.client_secret) > 0 {
				req.Header.Set("client_secret", configuration.client_secret)
			}
